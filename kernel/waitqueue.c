...
void awake_wait_queue(wait_queue_t* wait_queue, uint64 value){
    ...
    while(wait_queue->left != NULL){
        ...
        if(wait_node->thread != NULL && (wait_node->thread->waiting != wait_queue || wait_node->thread->state != T_SLEEPING)) {
            wait_queue_pop(wait_queue);
            continue;
        }
        
        if(wait_node->value != NULL) *wait_node->value = value;
        if(wait_node->thread != NULL) awake(wait_node->thread);
