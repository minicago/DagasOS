...
thread_t* alloc_thread(){
    ...
    free_thread_head = new_thread->next;
    new_thread->next = NULL;
    new_thread->state = T_PREPARING;
    release_spinlock(&thread_pool_lock);
    return new_thread;
}
...
int sys_wait(int pid){
    ...
    if(pid == -1) {
        ...
        thread->waiting = thread->process->wait_child;
    }
